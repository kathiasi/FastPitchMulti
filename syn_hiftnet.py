...
class Synthesizer:
    ...
    def __init__(self): 
        ...
        self.vocoder = hiftnet.HiFTNet(device=device)
    ...
    def speak(self, text, output_file="/tmp/tmp", spkr=0, lang=0, l_weight=1, s_weight=1, pace=0.95, clarity=1, guess_lang=True):
        ...
        for p in [0]:
            ...
            with torch.no_grad():
                ...
                mel, mel_lens, *_ = self.generator(text, pace, max_duration=15, speaker=spkr, language=lang, speaker_weight=s_weight, language_weight=l_weight)
            ...
            with torch.no_grad():
                ...
                y_g_hat = self.vocoder.synthesize_from_mel(mel)
...
if __name__ == '__main__':
    ...
    while (1==1):
        ...
        text1 = text.split(" ")
        for s in range(0, 20):
            syn.speak(text, output_file="/tmp/tmp", spkr=s, lang=1, clarity=1.5, guess_lang=False)
