...
def parse_args(parser):
    ...
    parser.add_argument('--fastpitch', type=str, default='output_sme_multispkr/FastPitch_checkpoint_400.pt',
                        help='Full path to the generator checkpoint file (skip to use ground truth mels)') #########    
    ...
    text_processing.add_argument('--symbol-set', type=str, default='sme_expanded', #################
                                 help='Define symbol set for input text')

    cond = parser.add_argument_group('conditioning on additional attributes')

    cond.add_argument('--n-speakers', type=int, default=4,
                      help='Number of speakers in the model.')
    cond.add_argument('--n-languages', type=int, default=2,
...
class Synthesizer:
    ...
    def speak(self, text, output_file="/tmp/tmp", lang=0, spkr=0, l_weight=1, s_weight=1, pace=0.95, clarity=1):
        ...
        lang = torch.LongTensor([lang]).to(device)
        spkr = torch.LongTensor([spkr]).to(device)
...
if __name__ == '__main__':
    ...
    sentences_file = "/home/hiovain/DeepLearningExamples/PyTorch/SpeechSynthesis/FastPitchMulti/quant_sents.txt"
    with open(sentences_file, 'r') as f:
        sentences = f.readlines()

    i = 0
    spkr = 2  # Only generate WAV files with speaker 2
    lang = 0
    for sentence in sentences:
        sentence = sentence.strip()
        text1 = sentence.split(" ")
        print("speaker", spkr, "language", lang)
        syn.speak(sentence, output_file=f"/home/hiovain/DeepLearningExamples/PyTorch/SpeechSynthesis/FastPitchMulti/inf_output_quant/{i}_{text1[0]}_{spkr}_{lang}_FP_{fastpitch_n_shortest}univnet", spkr=spkr, lang=lang)
        i += 1
